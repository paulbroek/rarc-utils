    ...
    def decorator(method):
        ...
        def ncalld(*args, **kw):
            ...
            if not log_once_per_n or lastAgo >= log_once_per_n:
                msg = '%r was called %d times in %d secs' % (method.__name__, ncall, log_once_per_n)
            ...
            if cnt >= max_:
                ...
                if return_type is not None: # the method has an annotated return type
                    ...
                    try:
                        ...
                        logger.error(f"cannot create default annotation instance to return, will return None. {e=!r}")
...
def timeit(log_once_per_n=None, reset_hist=False, save_hist=False):
    ...
    dq = lambda: deque(maxlen=MAX_HIST_LEN) # safer than list, for long running programs
        ...
        def timed(*args, **kw):
            ...
            if 'log_time' in kw:
                ...
                if not log_once_per_n or lastAgo >= log_once_per_n:
                    ...
                    msg = '{!r:<25} {:<7.2f} ms (min={:.2f}, max={:.2f}, n={:,})'.format(func_name, elaps_ms, min_elaps, max_elaps, len(elaps_hist))
...
def reconnect(func: Callable) -> Callable:
    ...
    def _reconnect(*args, **kwargs):
        ...
        assert 'db' in bound.arguments.keys(), f"decorator cannot be applied to method '{func.__name__}', misses 'db' kwarg"
        self = args[0]
        ...
        if len(job_matches) > 0:
            ...
            for job_key_, job_list in job_matches.items():
                for job_pars_ in job_list:
                    ...
                    match_rsel_str = match_rsel if '*' not in match_rsel else match_rsel.split('*')[0]

                    #print(f'{job_key=} {job_key_=} {rsel_str=} {match_rsel_str=}')

                    # find similar job
                    if not kwargs.get('recursive', False) and job_key_==job_key and rsel_str==match_rsel_str:
...
def colsAdded(x_or_func=None, return_cols=0, print_less_than=5, debug=0) -> Callable:
    ...
    def _colsAdded(func):
        ...
        def wrapper(*args, **kwargs) -> Tuple[pd.DataFrame, pd.Index]:
            ...
            added = cols_after.difference(cols_before, sort=False) # this however, does not return columns in the order they were added to df. how to fix? --> Use sort=False
            ...
            if len(added) > 0:
                ...
                COLS_ADDED = '' if len(added) > print_less_than else f": {', '.join(added)}"
...
def rowsReduced(func):
    ...
    def _rowsReduced(*args, **kwargs):
        ...
        if rows_dropped > 0:
            ...
            logger.info(f'reduced df by {rows_dropped:,} {ROWS} ({pct_rows_dropped:.2%})')
